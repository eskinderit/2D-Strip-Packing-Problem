...
def build_sb_constraints(solver, lp_instance, delta, x, y, H):
    ...
    for i in large_rectangles:
        ...
        solver.Add(delta[i[0]][i[1]][1] + delta[i[1]][i[0]][1] == 1)
    ...
    for i in smaller_rectangles:
        ...
        #solver.Add(delta[i][biggest_rectangle][0] == 1)
        #solver.Add(delta[biggest_rectangle][i][0] == 0)
...
def plot_LP_benchmark(instances_to_solve: int = 40, solver_name: str = "gurobi", timeout: int = 300, plot=False):
    ...
    #plt.yscale("log")
    plt.legend()
    plt.savefig(f'../../out/lp/lp_benchmark_{solver_name}.png', transparent=False, format="png")
    plt.show()
    ...
    print(out_text)
    # write txt log
    with open(f"../../out/lp/lp_benchmark_log_{solver_name}.txt", "w") as file:
        content = out_text
