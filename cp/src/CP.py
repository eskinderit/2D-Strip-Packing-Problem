            ...
            if method == "rotations" or method == "rotations-sb":
                ...
                for i in range(0, cp_instance.n_instances):
                    if rotated[i] == 1:
                        ...
                        cp_instance.rectangles[i].rotated = True
...
def plot_CP_benchmark(instances_to_solve: int = 40, solver_name: str = "chuffed", timeout: int =300, plot=False):
    ...
    #plt.yscale("log")
    plt.legend()
    plt.savefig(f'../../out/cp/cp_benchmark_{solver_name}.png', transparent=False, format="png")
    plt.show()
    ...
    print(out_text)
    # write txt log
    with open(f"../../out/cp/cp_benchmark_log_{solver_name}.txt", "w") as file:
        content = out_text
